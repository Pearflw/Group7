...
```{r}
#To eliminate running time, we sample the ratings dataset with 2,000 books and  2000 users
#Creating a subset with the 2,000 books and 2,000 users
ratings <- fread('ratings.csv')
x = data.frame(table(ratings$book_id) %>% sort(decreasing =T))
colnames(x) = c("book_id", "Popularity(Number of ratings)")
set.seed(7)
indices_x = sample(1:nrow(x), size = 4000, replace = FALSE)
x <- x[indices_x,]
x$book_id<-as.numeric(as.character(x$book_id))
the_books = merge(ratings, x, by="book_id")

y = data.frame(table(the_books$user_id) %>% sort(decreasing =T))
colnames(y) = c("user_id", "Number of books this user rated")
y = y[y$`Number of books this user rated`>=40,]
set.seed(7)
indices_y = sample(1:nrow(y), size = 6000, replace = FALSE)
y <- y[indices_y,]
y$user_id <- as.numeric(as.character(y$user_id))
ratings1 = merge(the_books, y, by="user_id")

z <- data.frame(table(ratings1$book_id) %>% sort(decreasing =T))
colnames(z) = c("book_id", "Popularity(Number of ratings)")
z <- z[z$`Popularity(Number of ratings)`>=30,]
z$book_id<-as.numeric(as.character(z$book_id))
ratings2 <- merge(ratings1, z, by="book_id")

ratings<-ratings2[,1:3]

#input ourselves into the rating dataset
#user_c <- head(ratings,20)
#user_c$user_id= 66666
#user_c$book_id <- c(10,1015,11,4,5,8,13,33,42,43,48,55,66,35,172,121,180,68,257,61)
#user_c$rating <- c(4,5,5,5,4,1,5,5,5,5,5,5,4,3,5,5,5,3,3,4)

#user_y <- head(ratings,20)
#user_y$user_id= 77777
#user_y$book_id <- c(5,94,80,1,59,106,128,247,11,67,863,130,10,439,162,359,635,2,103,514)
#user_y$rating  <- c(5,5,5,5,4,4,4,4,5,4,4,5,3,4,5,4,3,5,4,5)

#ratings<-rbind(ratings,user_c,user_y)
```
...
#try with an example: suggest 10 books to user whose ID is 2304
suggestBooks(bookDF, userDF, 329, 10)
```
...
library(arules)
ratings_full <- fread('ratings.csv')

#input ourselves into the rating dataset
user_c <- head(ratings_full,20)
user_c$user_id= 66666
user_c$book_id <- c(10,1015,11,4,5,8,13,33,42,43,48,55,66,35,172,121,180,68,257,61)
user_c$rating <- c(4,5,5,5,4,1,5,5,5,5,5,5,4,3,5,5,5,3,3,4)

user_y <- head(ratings_full,20)
user_y$user_id= 77777
user_y$book_id <- c(5,94,80,1,59,106,128,247,11,67,863,130,10,439,162,359,635,2,103,514)
user_y$rating  <- c(5,5,5,5,4,4,4,4,5,4,4,5,3,4,5,4,3,5,4,5)

ratings_full<-rbind(ratings_full,user_c,user_y)

joined_ratings = left_join(x=ratings_full,y=books_with_id,by="book_id") 
...
user_history <- ratings_full[ratings_full$user_id==329 &ratings_full$rating>=4]
...
x = data.frame(table(ratings$book_id) %>% sort(decreasing =T))
colnames(x) = c("book_id", "Popularity(Number of ratings)")
x <- x[x$`Popularity(Number of ratings)`>=30,]
set.seed(7)
indices_x = sample(1:nrow(x), size = 5000, replace = FALSE)
x <- x[indices_x,]
x$book_id<-as.numeric(as.character(x$book_id))
ratings0 = merge(ratings, x, by="book_id")

y = data.frame(table(ratings0$user_id) %>% sort(decreasing =T))
colnames(y) = c("user_id", "Number of books this user rated")
y = y[y$`Number of books this user rated`>=30,]
set.seed(7)
indices_y = sample(1:nrow(y), size = 5000, replace = FALSE)
y <- y[indices_y,]
y$user_id <- as.numeric(as.character(y$user_id))
ratings1 = merge(ratings0, y, by="user_id")

ratings<-ratings1[,1:3]

#to see the distribution of the rating matrix
ratings[, 'Number of books this user rated' := .N, .(user_id)] 
ratings[, 'Popularity(Number of ratings)' := .N, .(book_id)]

ratings <- ratings[ratings$`Number of books this user rated`>=30,]

ratings[, 'Number of books this user rated' := .N, .(user_id)] 
ratings[, 'Popularity(Number of ratings)' := .N, .(book_id)]

ratings <- ratings[ratings$`Popularity(Number of ratings)`>=30,]

ratings[, 'Number of books this user rated' := .N, .(user_id)] 
ratings[, 'Popularity(Number of ratings)' := .N, .(book_id)]

ratings <- ratings[ratings$`Number of books this user rated`>=20,]
...
write.csv(books_with_title,file="books_with_title.csv",quote=F,row.names = F)
```
...
for (i in c(0.001,0.003,0.005,0.007,0.01)){
  ...
  Rec.SVDF <- Recommender(getData(e, "train"), "SVDF", param=list(k=10,gamma=0.02,lambda=i,
...
set.seed(7)
e <- evaluationScheme(r, method="cross-validation",k=10, given=-5)
for (i in c(1,3,5,7,10,13,14,15,16,17,20,30)){
  SVDF_add<-data.frame(N_features = 0, test_RMSE=0) 
  Rec.SVDF <- Recommender(getData(e, "train"), "SVDF", param=list(k=i,gamma=0.02,lambda=0.005,
                                                                  min_epochs=1,max_epochs=100,min_improvement=1e-05,verbose=T))
  #making predictions on the test data set
  p_SVDF_test <- predict(Rec.SVDF, getData(e, "known"), type="ratings")
  # obtaining the error metrics for both approaches and comparing them
  test_RMSE_ibcf <-calcPredictionAccuracy(p_SVDF_test, getData(e, "unknown")) 
...
plot(SVDF_N,type="b")
#the result shows we use number of featrues = 16
```
...
plot(SVDF_N,type="b")
#the result shows we use number of featrues = 10
```
...
Algorithm <-list("UBCF_40"  = list(name="UBCF", param=list(nn = 40)),
                 "IBCF_2000"  = list(name="IBCF", param=list(k  = 2000)),
                 "Popular"  = list(name="POPULAR", param=NULL),
                 "Random Items" = list(name="RANDOM", param=NULL),
                 "SVD" = list(name="SVD", param=NULL),
                 "SVDF"=list(name="SVDF", param=list(k=16,gamma=0.02,lambda=0.005,
                                                                  min_epochs=20,max_epochs=100,min_improvement=1e-05)))
...
```{r}
#select some random user (from 1 to 2000) to do recommendation
userForPrediction<-2002    #2001 for Catalina, 2002 for Yanhan
#record his/her real user_id
Real_User_id <-dimension_indices[[1]][userForPrediction]
```
...
pred_IBCF <- predict ( r_recom_IBCF , r [userForPrediction] , type="ratings") 
...
r_recom_UBCF <- Recommender ( r , method = "UBCF" , param=list(nn = 40))
# predict 5 books based on UBCF
pred_UBCF <- predict ( r_recom_UBCF , r [userForPrediction] , type="ratings") 
...
# do predictions
pred_SVD <- predict ( r_recom_SVD , r [userForPrediction] , type="ratings") 
...
r_recom_SVDF <- Recommender (r, method = "SVDF" , param=list(k=16,gamma=0.02,lambda=0.005,
                                                                  min_epochs=20,max_epochs=100,min_improvement=1e-05))
# do predictions
pred_SVDF <- predict ( r_recom_SVDF , r [userForPrediction] , type="ratings") 
...
fsvd<- funkSVD(r, k = 16, gamma = 0.02, lambda = 0.005,
  min_improvement = 1e-05, min_epochs = 20, max_epochs = 100,
  verbose = T)
#fsvd$U
#fsvd$V
```
...
```{r}
scheme <- evaluationScheme(r, method="split", 0.8 , given=-5,  goodRating=5)

results_IBCF <- evaluate(scheme, method="IBCF",param=list(k  = 2000), type = "topNList",  n=c(3,5,10,15,20))
results_UBCF <- evaluate(scheme, method="UBCF",param=list(nn = 40)  , type = "topNList",  n=c(3,5,10,15,20))
results_SVDF <- evaluate(scheme, method="SVDF",param=list(k=16,gamma=0.02,lambda=0.005,
                                                          min_epochs=20,max_epochs=100,min_improvement=1e-05),
                         type = "topNList",  n=c(3,5,10,15,20))
...
```{r}
table<- as.data.frame(getConfusionMatrix(results_SVDF)[[1]][,c('precision','recall'),drop=F])
table <- cbind(table,n=as.numeric(rownames(table)))
table

plot(table$n,table$precision,col='red',type="b",ylim=c(0,0.08),xlab = "n",ylab ="precision")
lines(table$n,table$recall,col='blue',type="b")
legend("topright",pch=c(15,15),legend=c("precision","recall"),col=c('red','blue'),bty="n")

ggplot()+geom_line(data = table,aes(x = n,y = precision,colour = "precision"),size=1)+
  geom_point(data = table,aes(x = n,y = precision,colour = "precision"),size=2)+
  geom_line(data = table,aes(x = n,y = recall,colour ="recall"),size=1) + 
  geom_point(data = table,aes(x = n,y = recall,colour = "recall"),size=2) +
   xlab("N")+ylab("Rate")
```
...
dim(ratings[ratings$user_id==52036,])
ratings[ratings$user_id==901,]
...
The_2000_books<-data.frame(table(ratings$book_id) %>% sort(decreasing =T))
colnames(The_2000_books) = c("book_id", "Popularity(Number of ratings)")
books_with_title <- merge(x=The_2000_books,y=books_with_id,by="book_id")
...
z <- data.frame(table(ratings1$book_id) %>% sort(decreasing =T))
colnames(z) = c("book_id", "Popularity(Number of ratings)")
z <- z[z$`Popularity(Number of ratings)`>=20,]
z$book_id<-as.numeric(as.character(z$book_id))
ratings2 <- merge(ratings1, z, by="book_id")
