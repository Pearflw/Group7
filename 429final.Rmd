...
user_history <- ratings[ratings$user_id==77777 &ratings$rating>=4]
...
#use this to run in one train-test split
...
fsvd<- funkSVD(r, k = 8, gamma = 0.05, lambda = 0.005,
  min_improvement = 1e-06, min_epochs = 10, max_epochs = 200,
  verbose = T)
#fsvd$U
#fsvd$V
```
...
########################################
set.seed(7)
scheme <- evaluationScheme(r, method="split", train=0.8 , given=-5,  goodRating=5)
...
table_precision <- cbind(table1[,1],table2[,1],table3[,1])
colnames(table_precision) <- c('IBCF','UBCF','SVDF')
table_precision <-as.data.frame(table_precision)
table_precision$N <- as.factor(c(3,5,10,15,20,30))

table_recall <- cbind(table1[,2],table2[,2],table3[,2])
colnames(table_recall) <- c('IBCF','UBCF','SVDF')
table_recall <-as.data.frame(table_recall)
table_recall$N <- as.factor(c(3,5,10,15,20,30))

Comparision <- function(df,txt){
dfm <- melt(df[,c('N','IBCF','UBCF','SVDF')],id.vars = 1)
ggplot(data=dfm, aes(x=N, y=value, fill=variable)) +
    geom_bar(colour="black", stat="identity",
             position=position_dodge(),size=.3) +                        
    xlab("Number of recommendations") + ylab("Rate") +
    ggtitle(txt) + theme_bw()
}

Comparision(as.data.frame(table_precision),'Precision for 3 methods')
Comparision(as.data.frame(table_recall),'Recall for 3 methods')
...
table<- as.data.frame(getConfusionMatrix(results)[[1]][,c('precision','recall'),drop=F])
...
plot_results(results_IBCF)
plot_results(results_UBCF)
plot_results(results_SVDF)
```
